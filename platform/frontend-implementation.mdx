...
5. `PrimeVue` for dense admin/table/filter patterns only when existing local components and wrappers are insufficient
6. `Inspira UI` for visual inspiration and motion ideas
7. feature-local specialist libraries only after checking that the current stack and wrappers are insufficient
...
- a reference for dense admin layouts
- a reference for data-table, filter bar, and journal interactions
- an optional specialist source only after checking whether the same surface can be built with local components and wrappers first

Do not treat `PrimeVue` as the default visual layer of the dashboard, and do not let raw `PrimeVue` components become the public UI API of a feature.
