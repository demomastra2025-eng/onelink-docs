...
- a native shared scheduling and kassa capability with backend services under `app/services/scheduling/` and dashboard pages under `app/javascript/dashboard/routes/dashboard/scheduling/`
...
- source component ideas in this order:
  ...
  3. `VueUse` for composables, directives, browser APIs, and lightweight interaction behavior
  4. `reka-ui` for headless accessible primitives
  5. `originui-vue` for copy-paste Vue component composition and polished Tailwind app patterns
  6. `shadcn-vue` for recipes and component patterns
...
1. harden and iterate on the native scheduling and kassa module only where real product gaps remain
2. finish Phase 1 account profile/capability plumbing
3. stabilize Phase 2 shared CRM foundation around `Company` and related access/UI contracts
4. ship Phase 3 `Deal` MVP
5. ship Phase 4 `Task` MVP
