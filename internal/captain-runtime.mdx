...
- `Captain::ToolPolicy.runtime_allowed?` is the visibility/prompt gate: it keeps prompt references and selected-tool UX backward-compatible by checking scope and required account features
- actual tool calls also pass through the stricter execution gate `Captain::ToolPolicy.execution_allowed?`: it enforces runtime scope, required features, required operator/agent permissions, and agent high-risk runtime policy before mutation or external execution
- assistant-scope tools still use `Captain::Copilot::ToolConfirmationGate` for confirmation-required actions after the execution policy confirms the operator/tool is allowed
- agent-scope built-in, delegated registry, MCP, and custom HTTP tools are blocked at execution time if the account has not enabled the required permissioned/high-risk runtime policy
...
- CRM deal tools are pipeline-aware. `list_deal_pipelines` returns active account pipelines with ordered stages and stage IDs; `list_deal_stages` returns stages for a pipeline/current deal plus previous/next stage by `position`. When `create_deal`, `search_deals`, `update_deal`, or `transition_deal_stage` is prompt-referenced for the live agent/scenario runtime, the stage catalog tools are added as companions so the agent can resolve duplicate stage names and call mutations with `stage_id` instead of guessing.
- `transition_deal_stage(stage_action: "next"|"previous")` moves the current deal by stage `position` inside its current pipeline; explicit `stage_name`/`stage_code` lookups are scoped to the current pipeline unless `pipeline_id` or `pipeline_code` is supplied.
