---
title: How to enable IP identification in Onelink?
description: Guide to enabling IP address geocoding and location identification in the Onelink fork
sidebarTitle: Enable IP Logging
---

The fork lets you identify the user's location by geocoding the IP address. For IP address geocoding, the stack supports MaxmindDB services. This lookup provides methods for geocoding IP addresses without calling a remote API every time. You can follow the steps below to set up your self-hosted instance with geocoding.
...
With this step, the application would automatically download the [MaxmindDB downloadable databases](https://dev.maxmind.com/geoip/geoip2/downloadable/) and cache them locally.
