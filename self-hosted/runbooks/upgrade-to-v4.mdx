...
description: "Complete guide for upgrading the Onelink/Chatwoot fork to v4 with PostgreSQL pgvector support"
---

Version 4 of the fork requires PostgreSQL with `pgvector` support. You need to ensure that the version of PostgreSQL you are running supports `pgvector` before upgrading.
...
2. After verifying that the existing installation works with the pgvector image, proceed with the regular upgrade steps.
   [Learn more](https://one-link.kz/docs/self-hosted/deployment/upgrade#docker)
