...
<Note>
If you have already setup [Google OAuth login flow](https://one-link.kz/self-hosted/configuration/environment-variables#google-oauth) You can use the same app, by simply adding the new callback URL. **Do not remove the previous callback URL.**
</Note>
