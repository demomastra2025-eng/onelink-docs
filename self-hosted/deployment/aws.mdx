...
description: Deploy the Onelink fork on AWS with a reference HA architecture
...
```bash
wget https://one-link.kz/linux/install.sh
...
13. Also modify the other necessary environment variable for your chatwoot setup. Refer to https://one-link.kz/docs/self-hosted/deployment/linux-vm#configure-the-required-environment-variables
...
1. Refer to https://one-link.kz/docs/self-hosted/monitoring/apm-and-error-monitoring

## Updating OneLink

1. log in to one of the application servers and complete the update instructions. Run migrations if needed. Refer to https://one-link.kz/docs/self-hosted/deployment/linux-vm#upgrading-to-a-newer-version-of-chatwoot
