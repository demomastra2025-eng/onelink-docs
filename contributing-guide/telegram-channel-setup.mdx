...
description: Setup Telegram integration on your local machine for Onelink development
...
- Running the Onelink development environment

## Setup Steps

### 1. Start Ngrok Server

Start a Ngrok server listening at port `3000` or the port where you will be running the Onelink installation:
...
Update the `.env` variable `FRONTEND_URL` in the fork with the `https` version of the Ngrok URL:
...
### 3. Start Onelink Server

Start the server and create a new Telegram channel with the token obtained from Telegram BotFather.
...
1. **Access Onelink**: Go to your local instance (http://localhost:3000)
